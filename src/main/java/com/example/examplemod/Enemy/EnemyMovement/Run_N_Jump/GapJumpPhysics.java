...
final class GapJumpPhysics {
    ...
    /**
     * Відстань від центру блока до його передньої межі вздовж осі стрибка.
     */
    static final double EDGE_FRONT = 0.5;
    /**
     * Найпізніша точка відриву: центр моба на самій передній межі блока. Зазвичай правило
     * {@link #shouldTakeOff} спрацьовує трохи раніше (запас на прискорення) — типово центр моба
     * опиняється в зоні +0.2..+0.6 від центру блока, тобто хітбокс уже на самому краї, як у
     * гравця при стрибку "з краю". Це навмисно НЕ те, що було раніше: старий код ловив моба в колі
     * R=0.7 навколо ЦЕНТРУ блока, тобто відривав його ще на 0.2 блока ДО блока-краю (більш ніж за
     * 1.2 блока до реального краю).
     */
    static final double PREFERRED_TAKEOFF_ALONG = 0.5;
    /**
    ...
    // =====================================================================================
    // КОЛИ ВІДРИВАТИСЬ (геометрія краю)
    // =====================================================================================
    // Усі величини — вздовж осі стрибка, від ЦЕНТРУ блока-краю (along = 0), у бік приземлення.
    // Передня межа блока-краю: along = +0.5.
     ...
     *
     * @param along       позиція моба вздовж осі стрибка від центру блока-краю
     * @param speedAlong  поточна швидкість моба вздовж осі (deltaMovement, вже після тертя)
     ...
     */
    static boolean shouldTakeOff(double along, double speedAlong, double halfWidth, double groundAccel) {
        if (along >= PREFERRED_TAKEOFF_ALONG) {
            return true;
        }
        ...
        // Моб стоїть на землі, доки його хітбокс хоч трохи перекриває блок: центр <= EDGE_FRONT + halfWidth.
        double supportLimit = EDGE_FRONT + halfWidth - GROUND_LOSS_MARGIN;
        return along + predictedStep * STEP_LOOKAHEAD_FACTOR >= supportLimit;
    }
}